...
*&---------------------------------------------------------------------*
* PROGRAM DETAILS
* Program Name     : ZAPPOINTMENT_SAP_OUTLOOK
* Title            : ABAP Appointment SAP Outlook
*&---------------------------------------------------------------------*
...
go_appointment->set_location_string( 'Room POA' ).
go_appointment->set_type( 'ABSENT' ).
gs_text-line = |Yep! It works! { sy-datum } { sy-uzeit }|.
APPEND gs_text TO gt_text.
go_appointment->set_text( gt_text ).
gv_title = |Test ABAP appointment { sy-datum } { sy-uzeit }|.
